...
class ProfileLearner(Learner):
    ...
    def __init__(self, N, profile_size, ngram_type, similarity=None, **kwargs):
    ...
    def classify(self, song_df):
        ...
        if self.similarity == '-m':
            return {t: group.loc[np.argmin(group.apply(lambda row: manhatten(row['profile'], get_profile(song_df, self.N, self.ngram_type)), axis=1))]['name'] for t, group in self.types_grouped}
        elif self.similarity == '-d':
            return {t: group.loc[np.argmax(group.apply(lambda row: dice(row['profile'], get_profile(song_df, self.N, self.ngram_type)), axis=1))]['name'] for t, group in self.types_grouped}
        else:
