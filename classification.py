...
def learn_with_profiles(input_data_file):
    ...
    df['profile'] = df.apply(lambda row: get_profile(pd.read_csv("unigram/" + str(row.name) + ".csv", index_col=0)), axis=1)  # Make profile for each song in input
    ...
    return lambda song_df: {t: group.loc[np.argmax(group.apply(lambda row: similarity(row['profile'], get_profile(song_df)), axis=1))]['name'] for t, group in types_grouped}
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        classifier = learn_with_profiles(training_data_file)
